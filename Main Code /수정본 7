...
def gain_experience(page,exp):
    ...
    if current_exp >= max_exp:
            ...
            current_exp -= max_exp
            max_exp += 100
            label_leveltext.config(text=f"레벨업!\n현재 레벨:{current_level}",font=("한컴 윤고딕 240", 15))
...
def change_health(damage):
    ...
    if health<0:
        health=0
    label_health.config(text=f"현재 체력:{health}",font=("한컴 윤고딕 240", 15))
    
def give_reward(page,exp,damage):
    global health
    if health>=100 and damage>0:
        label_exp = tk.Label(page,text="체력이 최대입니다.",font=("한컴 윤고딕 240", 15))
        label_exp.pack()
        label_exp.after(1500,label_exp.destroy)
    elif health==0 and damage<0:
        label_exp = tk.Label(page,text="체력이 0입니다.",font=("한컴 윤고딕 240", 15))
        label_exp.pack()
        label_exp.after(1500,label_exp.destroy)
    else:
...
btn_start = tk.Button(page1, text="시작하기", width=20, height=2, font=("한컴 윤고딕 240", 18),
...
label_health = tk.Label(page3, text=f"현재 체력:{health}", font=("한컴 윤고딕 240", 15))
label_health.place(x=20, y=100)

level_bar = canvas_game.create_rectangle(0, 0, 0, 30, fill="limegreen", outline="")

#상호작용 버튼 설정
food_button=tk.Button(page3,text="밥 먹이기",font=("한컴 윤고딕 240",15),command=lambda:give_reward(page3,10,15))
food_button.pack(side="left",padx=30)

play_button=tk.Button(page3,text="놀아주기",font=("한컴 윤고딕 240",15),command=lambda:give_reward(page3,10,-15))
play_button.pack(side="left",padx=30)

# ---4번 화면 구성 ---
label4 = tk.Label(page4, text="게임", font=("한컴 윤고딕 240", 15), bg="white")
label4.pack(pady=50)
...
btn_real_start = tk.Button(page5, text="게임 시작", font=("한컴 윤고딕 240", 15),
